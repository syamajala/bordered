    ...
    def compute_basis(self, spinc=0):
        ...
        if __name__ == '__main__':
            ...
            if self.arcslid:
                ...
                for i in Combinations(str, 2).list():
                    if self.mstrands(i):
                        strands.append(i)
...
#            pool.join()
    ...
    def zero_type_D(self):
        ...
        if __name__ == '__main__':
            ...
            ids = manager.dict(ids)
            a = [(self, i, ids, 0) for i in a]

            ans = pool.map(alg_element, a, chunksize=1)
                                
            for i in ans:
                sa.extend(i)
            
            dgens = manager.list(gens)
            args = [(i, dgens) for i in sa]
                    
            diffsr = pool.map(self.diffr, args, chunksize=1)
            
            args = [(i, dgens) for i in diffsr if i != None]
            
            diffsl = pool.map(self.diffl, args, chunksize=1)
    ...
    def chords(self, spinc=0):
        ...
        if __name__ == '__main__':
            ...
            ans = pool.map(alg_element, arc_list, chunksize=1)
...
class PTypeDStr(TypeDStr):
    ...
    def mor_to_d(self, other):
        ...
        if __name__== '__main__':
            ...
            gens = pool.map(self.generate_gens, args, chunksize=1)                                                

            ans = alg_gens
            mult = []
            for i in range(2, self.pmc.punctures + 2):                
                ans = pool.map(self.mult, ans, chunksize=1)
            ...
            ans = pool.map(self.generate_gens, args, chunksize=1)
...
class PUnderslide(Underslide):
    ...
    def generate_gen_idems(self):
        ...
        if __name__ == '__main__':
            ...
            ids = pool.map(self.complementary_idem, pmc1idems, chunksize=1)

            for i in ids:
                x, y = i
                xidems.append(([x], y))
                                            
            ids = manager.dict([(i,frozenset(v)) for i,v in ids])
                
            pmc1idems = [i for i in self.pmc_1.idempotents() if len(i) != 1]
            args = [(i, ids) for i in pmc1idems]
            
            xidems.extend(pool.map(self.build_comp_idem, args, chunksize=1))
        ...
        #Generators of type Y (sub-complementary)                                                                                                                                                                                            
            ...
            yidems = pool.map(self.almost_complementary_idem, args)
...
class PTypeDDStr(TypeDDStr):
    ...
    def mor_to_d(self, other):
        ...
        if __name__ == '__main__':
            ...
            mor_basis = pool.map(self.compute_basis, args)
            ...
            mor_basis.extend(pool.map(self.compute_basis, args))            
